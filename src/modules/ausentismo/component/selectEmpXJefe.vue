<template>
	<div class="relative w-full">
		...
		<div
			...
			<div
				v-if="EmpXJefe.length > 0"
				v-for="(tipo, index) in EmpXJefe"
				:key="index"
				...
				<div
					@click="
						...
						emp_selected = tipo.NombreUsuario;
					"
...
import { EmpleadoXJefeI, TipoAusentismoI } from "../types/ausentismo.type";

type emit = {
	(event: "buscar", search: string): void;
	(event: "seleccionado", EmpXJefe: EmpleadoXJefeI): void;
};

interface pros {
	EmpXJefe: EmpleadoXJefeI[];
	placeholder: string;
}

const buscar = defineEmits<emit>();

const { EmpXJefe, placeholder } = defineProps<pros>();

// const clientsProcessed = ref<U[]>([]);

const emp_selected = ref("");
...
const onlyOpen = () => {
	...
	buscar("buscar", emp_selected.value);
};
